...
class BattleScreen(BattleAnimMixin, BattleDrawMixin):
    ...
    def __init__(self, screen, W, H, fonts, enemies, allies, enemy_formation="솔캐리_전방", ally_formation="트리오", gap=0.12):
        ...
        for enemy_name in enemies:
            ...
            if bgm_path and os.path.exists(bgm_path):
                try:
                    ...
                    reset_bgm_state()   # 전투가 음악을 직접 깔았음 → 다음 화면에서 BGM 재설정되게
                    break
                except Exception as e:
        ...
        # ── 비네팅 캐시 (한 번만 생성) ───────────────────────────
        # 효과를 약하게: 최대 알파를 낮추고(100→45), 지수를 높여(2→3)
        # 어두움을 화면 가장자리 쪽으로만 모은다.
        vignette = pygame.Surface((W, H), pygame.SRCALPHA)
        cx, cy = W // 2, H // 2
        max_r = (cx ** 2 + cy ** 2) ** 0.5
        VIGNETTE_PEAK = 45    # 가장자리 최대 어두움 (이전 100)
        VIGNETTE_CAP  = 90    # 상한 (이전 200)
        for y in range(0, H, 2):
            for x in range(0, W, 2):
                dx, dy = x - cx, y - cy
                dist = (dx ** 2 + dy ** 2) ** 0.5
                ratio = dist / max_r
                alpha = int(VIGNETTE_PEAK * ratio ** 3)
                if alpha > 0:
                    a = min(alpha, VIGNETTE_CAP)
    ...
    def _mouse_over_ui(self, pos=None):
        """마우스가 '현재 화면에 실제로 그려진' 전투 UI 위에 있는지.
        True 면 휠 줌을 막는다. (메뉴가 접혀 있을 때는 막지 않는다.)"""
        if pos is None:
            pos = pygame.mouse.get_pos()
        # 감정(인스펙트) 창이 열려 있으면 화면 대부분을 덮으므로 항상 UI 로 간주
        if self._inspect_target() is not None:
            return True
        # 명령 UI 패널 — MENU/SKILL/DEFENSE/TARGET 상태에서만 그려진다
        if self.state in (self.STATE_MENU, self.STATE_SKILL,
                          self.STATE_DEFENSE, self.STATE_TARGET):
            if self._ui_rect().collidepoint(pos):
                return True
        # 스킬/수비/타겟 선택 패널 — 각 상태에서만 그려진다
        if self.state in (self.STATE_SKILL, self.STATE_DEFENSE, self.STATE_TARGET):
            if self._target_rect().collidepoint(pos):
                return True
        return False
    ...
    def _enemy_sprite_rect(self, i):
        ...
        if e.sprite:
            ...
            # 그려질 때와 동일하게 하단 투명 여백만큼 내려 맞춘다
            cache = getattr(self, "_enemy_cache", [])
            drawn = cache[i] if i < len(cache) else None
            gap = self._opaque_bottom_gap(drawn) if drawn else 0
            full.midbottom = (sx, sy + gap)
    ...
    def _ally_sprite_rect(self, i):
        ...
        if a.sprite:
            ...
            full = pygame.Rect(0, 0, sw, sh)
            cache = getattr(self, "_ally_cache", [])
            drawn = cache[i] if i < len(cache) else None
            gap = self._opaque_bottom_gap(drawn) if drawn else 0
            full.midbottom = (sx, sy + gap)
    ...
    def handle_event(self, event):
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if not self._mouse_over_ui(event.pos):
    ...
    def update(self, dt):
        ...
        HIT_DUR = 0.22
        for c in (self.logic.enemies + self.logic.allies):
            t = getattr(c, "hit_push_t", 0)
            if t > 0:
                c.hit_push_t = max(0.0, t - dt / 1000.0)
                ratio = c.hit_push_t / HIT_DUR          # 1→0
                # 움찔 위치 = 최초 밀림량 × 남은비율 (복귀)
                c.hit_push = getattr(c, "hit_push_max", 0) * ratio
                # 빨강 농도도 같은 비율 (draw 에서 hit_flash_t 로 읽음)
                c.hit_flash_t = c.hit_push_t
        ...
        # 스킬 모션 진행
            ...
            self._update_sprite_sequence(dt)   # 전용 모션 스프라이트 시퀀스
            if self.anim["type"] == "command":
                ...
                self._update_smite(dt)
            else:
                self._update_melee_rush(dt)
            # 연출이 끝났으면(anim 해제) 스프라이트 기본 복귀
            if self.anim is None:
                self._end_sprite_sequence()
