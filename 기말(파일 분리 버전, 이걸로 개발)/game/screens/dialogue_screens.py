...
class BattleScreen:
    ...
    def handle_event(self, event):
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_ESCAPE:
                ...
                if event.key in (pygame.K_UP, pygame.K_w):
                    ...
                    if skills:
                        ...
                        if self.logic.is_support(self.pending_skill):
                            self._do_support()
                        else:
            ...
            if self.state == self.STATE_MENU and ui.collidepoint(mx, my):
                ...
                for i in range(len(self.enemies)):
                    slot_rect = pygame.Rect(tr.left, tr.top + i * slot_h, tr.width, slot_h)
                    if slot_rect.collidepoint(mx, my) and i < len(self.enemies):
                        self._do_attack(i)
    ...
    def _do_attack(self, target_idx):
        ...
        primary = None
        if 0 <= target_idx < len(self.enemies) and self.enemies[target_idx].hp > 0:
            primary = self.enemies[target_idx]
        elif alive:
            primary = alive[0]
    ...
    def _do_support(self):
        """지원 스킬: 대상 선택 없이 즉시 발동"""
        actor = self.logic.current_actor()
        if actor is None or self.pending_skill is None:
            return
        skill = self.pending_skill
        self.logic.use_skill(actor, skill, primary_target=None)
        self.pending_skill = None
        self.logic.advance()
        self._sync_turn()
