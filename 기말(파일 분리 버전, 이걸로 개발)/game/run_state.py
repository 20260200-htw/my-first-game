...
class RunState:
    ...
    def __init__(self):
        ...
        self.visit_count = {}       # 구역별 방문 횟수 {지역명: 횟수}
        self.cur_visit = 0          # 현재 구간이 그 지역의 몇 번째 방문인지
    ...
    def enter_region(self, region):
        """지역을 골라 다음 구간 진입. 방문 횟수 증가 + 노드맵 생성."""
        self.segment += 1
        self.region = region
        self.visit_count[region] = self.visit_count.get(region, 0) + 1
        self.cur_visit = self.visit_count[region]
    ...
    def end_run(self):
        """회차 종료: 휘발 상태 비활성화.
        스킬은 save_data 에 영구 보존, 레벨/포인트/아이템은 다음 회차 시작 시 초기화."""
        self.active = False
