...
pygame.display.set_caption("AABB & 원형 충돌 시각화")
...
player_radius = player_rect.width // 2  # 반지름: 너비의 절반
player_speed = 5

static_rect = pygame.Rect(0, 0, 100, 100)
static_rect.center = (screen_width // 2, screen_height // 2)
static_radius = static_rect.width // 2  # 반지름: 너비의 절반
...
while True:
    ...
    # --- 충돌 감지 로직 ---
    # 원형 충돌 체크: 중심점 사이의 거리 계산
    dx = player_rect.centerx - static_rect.centerx
    dy = player_rect.centery - static_rect.centery
    distance = math.sqrt(dx**2 + dy**2)
    
    circle_colliding = distance <= (player_radius + static_radius)
    aabb_colliding = player_rect.colliderect(static_rect)

    # 배경색 설정 (원형 충돌 시 노란색)
    current_bg = YELLOW if circle_colliding else BG_COLOR
